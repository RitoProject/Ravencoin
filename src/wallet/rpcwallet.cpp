...
UniValue sendfromaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nSend an amount from a specific address to a given address. All rito change will get sent back to the from_address\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"from_address\"       (string, required) The rito address to send from.\n"
            "2. \"to_address\"            (string, required) The rito address to send to.\n"
