...
static const std::regex RITO_NAMES("^RITO$|^RITOCOIN$|^#RITO$|^#RITOCOIN$|^RVN$|^RAVEN$|^RAVENCOIN$|^#RVN$|^#RAVEN$|^#RAVENCOIN$");
...
{
    // Issuing an Asset must contain at least 2 CTxOut( Rito Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedAsset(std::string& strError) const {
    // Issuing a restricted asset must cointain at least 4 CTxOut(Rito Burn Tx, Asset Creation, Root Owner Token Transfer, and CNullAssetTxVerifierString)
    if (vout.size() < 4) {
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_RITO_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_RITO_ASSET OP_RITO_ASSET OP_RITO_ASSETS data transaction to the transaction
...
void CNullAssetTxData::ConstructGlobalRestrictionTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_RITO_ASSET << OP_RESERVED << OP_RESERVED << ToByteVector(vchMessage);
}
...
void CNullAssetTxVerifierString::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_RITO_ASSET << OP_RESERVED << ToByteVector(vchMessage);
}
