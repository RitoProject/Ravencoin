...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for RITO
    ...
    if (p_rewardSnapshot.strDistributionAsset != "RITO") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is RITO with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using RITO differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "RITO") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Rito address (already validated during ownership snapshot creation)
