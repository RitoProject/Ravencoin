...
bool CScript::IsAssetScript(int& nType, bool& fIsOwner, int& nStartingIndex) const
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_RITO_ASSET) { // OP_RITO_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == RITO_R) { // Check to see if RVN starts at 27 ( this->size() < 105)
                if ((*this)[28] == RITO_V)
                    if ((*this)[29] == RITO_N)
                        index = 30;
            } else {
                if ((*this)[28] == RITO_R) // Check to see if RVN starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == RITO_V)
                        if ((*this)[30] == RITO_N)
                            index = 31;
            }
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_RITO_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_RITO_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_RITO_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_RITO_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_RITO_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_RITO_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
