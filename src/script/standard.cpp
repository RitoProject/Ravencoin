...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_RITO_ASSET && scriptPubKey.IsPushOnly(scriptPubKey.begin()+1)) {
...
namespace
{
    ...
    public:
        ...
        bool operator()(const CKeyID &keyID) const {
            script->clear();
            *script << OP_RITO_ASSET << ToByteVector(keyID);
            return true;
        }

        bool operator()(const CScriptID &scriptID) const {
            script->clear();
            *script << OP_RITO_ASSET << ToByteVector(scriptID);
            return true;
        }
    };
} // namespace
