...
//        toolbar->addAction(votingAction);
        ...
        QLabel* labelBtcRito = new QLabel();
        labelBtcRito->setText("BTC / RITO");
        labelBtcRito->setContentsMargins(15,0,0,0);
        labelBtcRito->setFixedHeight(75);
        labelBtcRito->setAlignment(Qt::AlignVCenter);
        labelBtcRito->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcRito->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/RitoProject/Ritocoin/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(labelBtcRito, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        // Get the latest Ritocoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/RitoProject/Ritocoin/releases";
...
void RitoGUI::gotoRestrictedAssetsPage()
{
...
static bool ThreadSafeMnemonic(RitoGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void RitoGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    for (const RitoUnits::Unit unit : units)
    {
        	...
        	max_width = qMax(max_width, fm.horizontalAdvance(RitoUnits::name(unit)));
        #else
...
void RitoGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/RitoProject/Ritocoin/releases"));
