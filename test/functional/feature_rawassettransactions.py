...
class RawAssetTransactionsTest(RitoTestFramework):
    ...
    def reissue_tampering_test(self):
        ...
        tx.deserialize(f)
        ritor = '72766e72'  # ritor
        op_drop = '75'
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if ritor in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                reissue_script_hex = script_hex[script_hex.index(ritor) + len(ritor):-len(op_drop)]
                ...
                tampered_script = script_hex[:script_hex.index(ritor)] + ritor + tampered_reissue + op_drop
        ...
        f = BytesIO(hex_str_to_bytes(tx_hex))
        tx.deserialize(f)
        ritot = '72766e74'  # ritot
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: ritot not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_hex_bad = bytes_to_hex_str(tx.serialize())
        tx_signed = n0.signrawtransaction(tx_hex_bad)['hex']
    ...
    def issue_tampering_test(self):
        ...
        tx.deserialize(f)
        ritoo = '72766e6f'  # ritoo
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: ritoo not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        ...
        owner_vout = list(filter(lambda out_script: ritoo in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))[0]
        ...
        f = BytesIO(hex_str_to_bytes(tx_issue_hex))
        tx.deserialize(f)
        ritoq = '72766e71'  # ritoq
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: ritoq not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-bad-asset-transaction",
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if ritoo in bytes_to_hex_str(out.scriptPubKey):
                owner_out = out
                owner_script_hex = bytes_to_hex_str(owner_out.scriptPubKey)
                asset_script_hex = owner_script_hex[owner_script_hex.index(ritoo) + len(ritoo):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                owner = CScriptOwner()
                owner.deserialize(f)
                owner.name = b"NOT_MY_ASSET!"
                tampered_owner = bytes_to_hex_str(owner.serialize())
                tampered_script = owner_script_hex[:owner_script_hex.index(ritoo)] + ritoo + tampered_owner + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        ...
        for n in range(0, len(tx.vout)):
            ...
            if ritoo in bytes_to_hex_str(out.scriptPubKey):
                ...
                tampered_script = owner_script_hex.replace(ritoo, RITOO)
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-rito-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def issue_reissue_transfer_test(self):
        ...
        for i in range(1, 3):
            ...
            f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(ritot) + len(ritot):-len(op_drop)]))
            ...
            tampered_script = script_hex[:script_hex.index(ritot)] + ritot + tampered_transfer + op_drop
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if ritot in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(ritot) + len(ritot):-len(op_drop)]))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
    ...
    def issue_sub_multiple_outputs_test(self):
        ...
        # Create the valid sub asset and broadcast the transaction
        outputs = {
            sub_burn: burn,            change_address: truncate(float(unspent['amount']) - (burn + 0.01)),
            owner_change_address: {
                'transfer': {
                    owner: 1,
                }
        ...
        for n in range(0, len(tx.vout)):
            ...
            if ritot in bytes_to_hex_str(out.scriptPubKey):
                transfer_out = out
                transfer_script_hex = bytes_to_hex_str(transfer_out.scriptPubKey)
                asset_script_hex = transfer_script_hex[transfer_script_hex.index(ritot) + len(ritot):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
                transfer.amount = 0
                tampered_transfer = bytes_to_hex_str(transfer.serialize())
                tampered_script = transfer_script_hex[:transfer_script_hex.index(ritot)] + ritot + tampered_transfer + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_transfer = bytes_to_hex_str(tx.serialize())
        tx_bad_transfer_signed = n0.signrawtransaction(tx_bad_transfer)['hex']
    ...
    def transfer_asset_inserting_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if ritot in bytes_to_hex_str(out.scriptPubKey):
                ...
                # Generate a script that has a valid destination address but switch it with ritoq and the issue_serialized data
                issue_out.scriptPubKey = hex_str_to_bytes(transfer_script_hex[:transfer_script_hex.index(ritot)] + ritoq + issue_serialized + op_drop)
    ...
    def fundrawtransaction_transfer_outs(self):
        ...
        for _ in range(0, 5):
            n0.sendtoaddress(n2_address, rito_amount / 5)
        n0.generate(1)
        self.sync_all()
